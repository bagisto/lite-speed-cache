{{--
    LSC override of shop::components.products.carousel (x-shop::products.carousel).

    Only `getProducts()` is changed: the API response is normalised to an array
    and any null/undefined entries are filtered out before being assigned to
    `products`. A null entry (e.g. a related/up-sell association whose product was
    deleted or disabled) would otherwise be passed to the products.card component
    and crash the render with "Cannot read properties of undefined (reading 'url_key')".
    The rest of the file is identical to the core view.
--}}
<v-products-carousel
    src="{{ $src }}"
    title="{{ $title }}"
                ...
                getProducts() {
                    this.$axios.get(this.src)
                        .then(response => {
                            ...
                            /**
                             * Guard against null/undefined entries in the response
                             * (e.g. a related/up-sell association whose product was
                             * deleted or disabled). Such an entry would be passed to
                             * the products.card component and crash its render on
                             * `product.url_key`.
                             */
