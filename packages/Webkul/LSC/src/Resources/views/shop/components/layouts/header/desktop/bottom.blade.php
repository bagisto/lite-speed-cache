...
@pushOnce('scripts')
    ...
    {{-- LSC ESI Mini Cart Component --}}
    <script
        type="text/x-template"
        id="v-lsc-mini-cart-esi-template"
    >
        <div class="lsc-esi-mini-cart relative">
            {{-- Cart Icon with Badge --}}
            <span class="relative cursor-pointer" @click="toggleDrawer">
                <span
                ...
                {{-- Cart Count Badge (ESI or AJAX loaded) --}}
                <span
                    v-if="cartCount > 0"
                ...
                <div class="absolute right-0 top-0 h-full w-full max-w-md bg-white shadow-xl">
                    <div class="flex h-full flex-col">
                        ...
                        <div class="flex items-center justify-between border-b p-4">
                            ...
                            <button @click="closeDrawer" class="text-2xl">&times;</button>
                        </div>

                        {{-- Content --}}
                        <div class="flex-1 overflow-auto p-4" v-html="cartContent">
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script type="module">
        app.component('v-lsc-mini-cart-esi', {
            template: '#v-lsc-mini-cart-esi-template',
            ...
            mounted() {
                ...
                this.$emitter.on('update-mini-cart', this.refreshCart);
            },

            methods: {
                loadCartCount() {
                    fetch('/esi/cart-count', {
                        headers: {
                    ...
                    .then(html => {
                        // Parse count from HTML
                        const match = html.match(/data-cart-(?:qty|count)="(\d+)"/);
                        if (match) {
                            this.cartCount = parseInt(match[1], 10);
                        }
                    })
                    .catch(error => {
                        console.warn('LSC ESI cart count error:', error);
                    });
                },
                ...
                loadMiniCart() {
                    ...
                    .catch(error => {
                        console.warn('LSC ESI mini cart error:', error);
                        this.isLoading = false;
                    });
                },
